using EPG.Samples.AuthAPI.Extensions;
...
namespace EPG.Samples.AuthAPI.Controllers
{
    ...
    public class AuthController : MainController
    {
        ...
        {
            if (!ModelState.IsValid) return GerarResposta(ModelState);
            ...
            {                
                return GerarResposta(await GerarTokenJWT(usuarioCadastro.Email));
            }
        ...
        public async Task<IActionResult> Login([FromBody] UsuarioLogin usuarioLogin)
        {
            ...
            if (resultado.Succeeded)
            {
                return GerarResposta(await GerarTokenJWT(usuarioLogin.Email));
            }
